...
def save_to_csv(items, filename):
    with open(filename, mode='w', newline='', encoding='utf-8') as file:
        ...
        for item in items:
            ...
            if not categories:  # If categories are empty
                categories = ['No categories available']
            # Convert the list to a string, preserving the original format
            categories = ''.join(categories)  # Use a semicolon to separate categories
